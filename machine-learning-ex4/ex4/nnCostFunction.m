...
%y_k = (1:num_labels)==y; %it is the logical array which gives output similar to  0100 where y will be the boolean 1  
J=(1/m)*sum(sum((-y_k.*log(h_x))-((1-y_k).*log(1-h_x))));
...
%%%%%% Backpropogation (Vectorized Implementation) %%%%%%%
  % Here X is including 1 column at begining, so no need to add 1
%layer1
A1 = X;
...
%%%%%%% Backpropogation using for loop %%%%%%%
  % for t=1:m
  %     % Here X is including 1 column at begining,  so no need to add 1 to X
  %     
  %     % for layer-1
...
%%%%%%%%%%%% SIMILAR CALCULATION OF THETA GRADIENT WITH REGULARISATION %%%%%%%%%%%
